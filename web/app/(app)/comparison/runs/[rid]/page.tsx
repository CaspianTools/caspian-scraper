...
export default async function ComparisonRunDetailPage({ params }: PageProps) {
  const session = await getSessionFromCookie();
  if (!session) redirect("/signin");
  const { rid } = await params;

  const runSnap = await comparisonRunsCol().doc(rid).get();
  ...
  const errors: string[] = Array.isArray(run.errors)
    ...
    : [];
  const diagnostics = (run.diagnostics ?? {}) as {
    ...
    <>
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
            Trigger: <code className="text-xs">{String(run.trigger ?? "—")}</code>
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        ...
        <Stat
          label="Finished"
          value={
            run.finished_at ? fmtAbsolute(tsToIso(run.finished_at)) : "—"
          }
          small
        />
        <Stat
        ...
        <Stat label="Extracted" value={String(totals.extracted ?? 0)} />
      </div>

      <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 p-5">
        <h3 className="font-medium mb-3">Extraction diagnostics</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          <Stat
            label="Links discovered"
            value={String(diagnostics.links_discovered ?? 0)}
          />
          <Stat
            label="Pages visited"
            value={String(diagnostics.pages_visited ?? 0)}
          />
          <Stat
            label="Extractor hits"
            value={
              Object.entries(diagnostics.extractor_hits ?? {})
                .filter(([, v]) => v > 0)
                .map(([k, v]) => `${k}:${v}`)
                .join(", ") || "—"
            }
          ...
          <Stat
            label="HTTP errors"
            value={
              Object.entries(diagnostics.http_errors ?? {})
                .map(([k, v]) => `${k}:${v}`)
                .join(", ") || "—"
            }
          />
        </div>
        {diagnostics.links_discovered === 0 && (
          <p className="text-xs text-amber-700 dark:text-amber-300 mt-3">
            No detail-page links were discovered. Check your{" "}
            <code>link_discovery</code> selector — it may not match the
            current listing-page HTML.
          </p>
        )}
        {Object.keys(diagnostics.http_errors ?? {}).length > 0 && (
          <p className="text-xs text-red-700 dark:text-red-300 mt-3">
            Upstream returned HTTP errors. 403/429 typically means the site
            is blocking automated access (proxy support arrives in Phase C).
          </p>
        )}
      </div>
