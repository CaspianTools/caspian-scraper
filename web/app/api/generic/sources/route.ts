...
/**
 * GET /api/generic/sources — the signed-in user's generic sources, newest
 * first. Equality-only query + in-memory sort so no composite index is
 * required.
 */
export async function GET(req: NextRequest) {
  ...
  const snap = await genericSourcesCol()
...
/**
 * POST /api/generic/sources — create a generic source owned by the
 * signed-in user. source_key must be unique per user (it's the record-uid
 * prefix — collisions would clash across sources).
 */
export async function POST(req: NextRequest) {
  ...
  const parsed = GenericSourceCreateSchema.safeParse(body);
  if (!parsed.success) {
    ...
    return NextResponse.json(
      {
        ...
        details: [
          {
            path: ["source_key"],
            message: `source_key '${input.source_key}' already used by another of your sources`,
          },
        ],
      },
      { status: 400 }
  ...
  const doc: GenericSourceDoc = {
    ...input,
    // Adapter-strategy sources reference a generated Python module that must be
    // merged to main before they can run — force inactive (mirrors the approve
    // route) so a raw create can't queue a source that errors every tick.
    active: input.strategy.mode === "adapter" ? false : input.active,
